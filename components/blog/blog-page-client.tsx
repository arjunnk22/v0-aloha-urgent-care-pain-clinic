...
export default function BlogPageClient() {
  ...
  return (
    <div className="bg-white min-h-screen">
      {/* Page Hero */}
      <section className="bg-white pt-32 pb-16 px-6 lg:px-8 relative overflow-hidden border-b border-black/10">
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
            <div>
              ...
              <h1 className="text-black text-4xl lg:text-6xl font-black leading-tight text-balance">
                The Aloha<br />
                <span className="text-[#d52128]">Health Blog</span>
              </h1>
            </div>
            <p className="text-black text-base leading-relaxed max-w-md font-[family-name:var(--font-inter)]">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
          ...
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-2.5 bg-white border border-black/10 rounded-sm text-sm text-black placeholder:text-black/40 focus:outline-none focus:border-[#d52128] focus:ring-1 focus:ring-[#d52128]/20 font-[family-name:var(--font-inter)] transition-all"
            />
            {query && (
              <button
                onClick={() => setQuery('')}
          ...
          {/* Divider */}
          <div className="hidden sm:block w-px h-6 bg-black/10" aria-hidden="true" />
          ...
          <div className="flex items-center gap-1 overflow-x-auto pb-0.5 sm:pb-0 scrollbar-none" role="tablist" aria-label="Filter by category">
            <SlidersHorizontal size={14} className="text-black/40 shrink-0 mr-1" />
            {categories.map((cat) => (
              <button
                key={cat}
                role="tab"
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-8 py-14">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-28 text-center">
            ...
            <h2 className="text-black text-xl font-bold mb-2">No articles found</h2>
            <p className="text-black/60 text-sm font-[family-name:var(--font-inter)] mb-6">
          ...
          <div className="flex flex-col lg:flex-row gap-12">
            ...
            <div className="flex-1 min-w-0">
              {/* Result count */}
              <p className="text-black/60 text-xs font-[family-name:var(--font-inter)] mb-6">
                Showing <span className="text-[#d52128] font-bold">{filtered.length}</span> article{filtered.length !== 1 ? 's' : ''}
                {activeCategory !== 'All' && (
            ...
            <aside className="lg:w-80 shrink-0 space-y-8">
              ...
              <div className="bg-white border border-black/10 rounded-sm p-6">
                <h2 className="text-black font-black text-sm uppercase tracking-widest mb-5 font-[family-name:var(--font-inter)]">
                  Latest Reads
                </h2>
                <div>
              ...
              <div className="bg-[#d52128] rounded-sm p-7 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -translate-y-8 translate-x-8 blur-2xl" aria-hidden="true" />
                <span className="text-white/70 text-xs font-bold uppercase tracking-widest font-[family-name:var(--font-inter)] block mb-3">
                  Ready to be seen?
                </span>
                <h3 className="text-white font-black text-xl leading-tight mb-3 text-balance">
                ...
                <a
                  href="/#contact"
                  className="block bg-white hover:bg-black/5 text-[#d52128] text-sm font-bold py-3 px-6 rounded-sm text-center transition-colors font-[family-name:var(--font-inter)]"
                >
                  Book Now
                </a>
                <div className="mt-4 pt-4 border-t border-white/15 flex flex-col gap-1">
                  <p className="text-white/60 text-xs font-[family-name:var(--font-inter)]">Mon–Thurs, Sat: 9:30 AM – 11:00 PM</p>
                  <p className="text-white/60 text-xs font-[family-name:var(--font-inter)]">Fri: 4:30 PM – 11:00 PM &nbsp;·&nbsp; Sun: Closed</p>
                </div>
              </div>

              {/* Categories cloud */}
              <div className="bg-white border border-black/10 rounded-sm p-6">
                <h2 className="text-black font-black text-sm uppercase tracking-widest mb-4 font-[family-name:var(--font-inter)]">
                  Browse by Topic
                </h2>
                  ...
                  {categories.filter((c) => c !== 'All').map((cat) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-sm text-xs font-semibold border transition-all font-[family-name:var(--font-inter)] ${
                        activeCategory === cat
                          ? 'bg-[#d52128] text-white border-[#d52128]'
                          : 'border-black/10 text-black/60 hover:border-[#d52128] hover:text-[#d52128]'
                      }`}
                    >
                      {cat}
                    </button>
