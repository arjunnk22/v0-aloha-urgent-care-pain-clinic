...
export default function WhyUs() {
  ...
  return (
    <section id="why-us" className="bg-white py-24 lg:py-32 relative overflow-hidden">
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            'radial-gradient(ellipse 50% 70% at 100% 0%, rgba(213,33,40,0.05) 0%, transparent 60%)',
        }}
        aria-hidden="true"
      />
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {/* Interactive Panel */}
        <div className="grid lg:grid-cols-[340px_1fr] gap-0 border border-black/10 rounded-sm overflow-hidden">
          {/* Tab List */}
          <nav
            className="flex flex-col overflow-y-auto max-h-[540px] border-r border-black/10 divide-y divide-black/5 bg-white"
                ...
                <button
                  ...
                  className={`group flex items-center gap-3 px-6 py-4 text-left transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#d52128] ${
                    isActive
                      ...
                      : 'bg-white text-black hover:bg-black/5 hover:text-black'
                  }`}
                  aria-selected={isActive}
                  role="tab"
                >
                  <TabIcon size={16} className={`shrink-0 ${isActive ? 'text-white' : 'text-[#d52128]'}`} />
                  <span className="text-sm font-semibold leading-snug font-[family-name:var(--font-inter)]">
                    {r.title}
                  </span>
                  {isActive && (
                    <ArrowRight size={14} className="ml-auto shrink-0 text-white/70" />
                  )}
                </button>
              )
            })}
          ...
          <div className="relative flex flex-col">
            ...
            {/* Content */}
            <div className="flex flex-col gap-4 p-8 lg:p-10 flex-1 bg-white">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-black font-black text-xl leading-tight">
                  {current.title}
                </h3>
              </div>
              ...
              <a
                href="#contact"
                className="group inline-flex items-center gap-2 bg-[#d52128] hover:bg-[#b91b21] text-white font-bold px-6 py-3 rounded-sm transition-all duration-200 text-sm font-[family-name:var(--font-inter)] mt-auto w-fit"
              >
                Book Now
        ...
        <div className="grid sm:grid-cols-2 gap-8 text-center max-w-2xl mx-auto">
          {[
            ...
            <div key={item.unit}>
              ...
              >
                {item.num}
                <span className="text-black font-semibold text-lg ml-1">{item.unit}</span>
              </p>
              <p className="text-black text-sm mt-2 font-[family-name:var(--font-inter)]">
                {item.label}
              </p>
            </div>
          ))}
